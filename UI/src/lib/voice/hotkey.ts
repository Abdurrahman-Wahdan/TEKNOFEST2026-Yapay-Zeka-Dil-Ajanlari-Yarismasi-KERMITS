 ...
 *
 * The space bar is a loaded key. It scrolls the page, it types a character, and
 * it activates a focused button or link -- so every one of those has to be
 * ruled out before it can mean "listen to me", and each `false` below is a
 * place the feature would otherwise break something that already worked.
 */

import type { ChatStatus } from "../chat/types";

/**
 * Where a space bar is a character, not a command.
 *
...
/** Elements the space bar already activates, where it must keep doing that. */
export const VOICE_ACTIVATES_SELECTOR = [
  "button",
  "a[href]",
  "summary",
  "[role='button']",
 ...
 *
 * The dock is on screen for the whole back half of a turn, and pressing Space
...
export type VoiceHotkeyContext = {
  ...
  pathname: string;
  /** Focus is somewhere a space bar types a character. */
  inTextEntry: boolean;
  /** Focus is on something a space bar already activates. */
  activatesOnSpace: boolean;
   ...
   * afterwards: pressing space to cut the assistant off and ask the next thing
...
export function shouldOpenVoiceMode(
  ...
  // Every modified space belongs to the browser or the OS, not to us.
  ...
  if (context.activatesOnSpace) return false;
  // The composer autofocuses on /chat, so a space there is nearly always a
