...
// Viewport width read through a hook: `window` does not exist during
// server rendering, and this component is rendered there too.
import useViewportWidth from "vision/useViewportWidth";
...
function ReferralTracking() {
  const viewportWidth = useViewportWidth();
	const { info, gradients, surfaces } = useTheme().palette;
				...
				<VuiBox
					...
					<VuiBox sx={{ position: 'relative', display: 'inline-flex' }}>
						<CircularProgress
							variant='determinate'
							value={70}
							size={viewportWidth >= 1024 ? 200 : viewportWidth >= 768 ? 170 : 200}
							color='success'
						/>
						<VuiBox
							sx={{
