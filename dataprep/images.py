...
MIN_PX = 100          # bu boyutun altındaki görseller ikon/dekoratif -> atla
MAX_BYTES = 8_000_000
...
def process_bank(slug: str, workers: int = WORKERS) -> None:
    ...
    cache = vlm.ImageCache(site / "_image_cache.json")
    ...
    def work(img_url: str):
        ...
        try:                                  # Referer = görselin sitesi (tarayıcı gibi)
            ...
            return img_url, None
        png = _to_png(content) if len(content) < MAX_BYTES else None
    ...
    with client, ThreadPoolExecutor(max_workers=workers) as ex:
        for j, (img_url, res) in enumerate(ex.map(work, todo), 1):
            results[img_url] = res
            ...
            if j % 100 == 0:                   # sık kaydet: kill olursa re-download israfı az
